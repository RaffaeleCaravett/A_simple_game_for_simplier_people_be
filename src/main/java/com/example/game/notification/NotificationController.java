...
public class NotificationController {
    ...
    @GetMapping("/read")
    public boolean readAll(@RequestBody @Valid List<Long> ids, BindingResult bindingResult, @AuthenticationPrincipal User user) {
        if (bindingResult.hasErrors()) {
            throw new BadRequestException(bindingResult.getAllErrors());
        }
        return notificationService.read(ids, user);
    }
